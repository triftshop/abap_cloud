CLASS zbp_generate_bookings_adr DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_adr IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_adr.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_adr.
*   insert the new table entries
    INSERT ztbooking_adr FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_adr INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
